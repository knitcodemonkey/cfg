...
function findFirefoxProfileDir(profileName) {
    return new Promise((resolve, reject) => {
        let finder = new firefox_profile_1.default.Finder();
...
function parseWebRootConfiguration(config, pathMappings) {
    if (config.url) {
        ...
        let webRoot = misc_1.normalizePath(config.webRoot);
...
function parseReloadConfiguration(reloadConfig) {
    ...
    if (typeof reloadConfig === 'string') {
        return {
            watch: [misc_1.normalizePath(reloadConfig)],
            ignore: [],
        ...
        return {
            watch: reloadConfig.map(path => misc_1.normalizePath(path)),
            ignore: [],
            debounce: defaultDebounce
        };
    }
        ...
        watch = watch.map((path) => misc_1.normalizePath(path));
        let ignore;
        ...
        ignore = ignore.map((path) => misc_1.normalizePath(path));
        let debounce;
...
function parseAddonConfiguration(config, pathMappings) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        let addonId = yield misc_1.findAddonId(addonPath);
