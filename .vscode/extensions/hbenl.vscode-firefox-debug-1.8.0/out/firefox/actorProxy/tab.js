...
class TabActorProxy extends events_1.EventEmitter {
    constructor(name, _title, _url, sourceMaps, pathMapper, connection) {
        super();
        this.name = name;
        this._title = _title;
        this._url = _url;
    ...
    receiveResponse(response) {
        if (response['type'] === 'tabAttached') {
            ...
            if (this.sourceMaps === 'client') {
                threadActor = new index_1.SourceMappingThreadActorProxy(threadActor, this.pathMapper, this.connection);
            }
            ...
            workersResponse.workers.forEach((worker) => {
                ...
                if (this.workers.has(worker.actor)) {
                    ...
                    workerActor = new index_1.WorkerActorProxy(worker.actor, worker.url, this.sourceMaps, this.pathMapper, this.connection);
