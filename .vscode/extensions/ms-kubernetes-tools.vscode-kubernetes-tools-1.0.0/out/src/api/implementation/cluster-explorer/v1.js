...
function resourceFolderContributor(displayName, pluralDisplayName, manifestKind, abbreviation) {
    const nodeSource = new explorer_1.CustomResourceFolderNodeSource(new kuberesources_1.ResourceKind(displayName, pluralDisplayName, manifestKind, abbreviation));
    return apiNodeSourceOf(nodeSource);
}
function groupingFolderContributor(displayName, contextValue, ...children) {
    const nodeSource = new explorer_1.CustomGroupingFolderNodeSource(displayName, contextValue, children.map(internalNodeSourceOf));
    return apiNodeSourceOf(nodeSource);
}
