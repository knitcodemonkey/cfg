...
function activate(context) {
    ...
    function openPreview(uri) {
        ...
        */
        const previewUri = preview_content_provider_1.getPreviewUri(resource);
        return vscode.commands.executeCommand('vscode.previewHtml', previewUri, vscode.ViewColumn.Two, preview_content_provider_1.useSinglePreview() ? 'MPE Preview' : `Preview '${path.basename(resource.fsPath)}'`)
            .then((success) => {
    ...
    function customizeCSS() {
        const globalStyleLessFile = mume_1.utility.addFileProtocol(path.resolve(mume_1.utility.extensionConfigDirectoryPath, './style.less'));
        vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(globalStyleLessFile));
    }
    function openMermaidConfig() {
        const mermaidConfigFilePath = mume_1.utility.addFileProtocol(path.resolve(mume_1.utility.extensionConfigDirectoryPath, './mermaid_config.js'));
        vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(mermaidConfigFilePath));
    }
    function openMathJaxConfig() {
        const mathjaxConfigFilePath = mume_1.utility.addFileProtocol(path.resolve(mume_1.utility.extensionConfigDirectoryPath, './mathjax_config.js'));
        vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(mathjaxConfigFilePath));
    }
    function openPhantomJSConfig() {
        const phantomjsConfigFilePath = mume_1.utility.addFileProtocol(path.resolve(mume_1.utility.extensionConfigDirectoryPath, './phantomjs_config.js'));
        vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(phantomjsConfigFilePath));
    }
    function extendParser() {
        const parserConfigPath = mume_1.utility.addFileProtocol(path.resolve(mume_1.utility.extensionConfigDirectoryPath, './parser.js'));
        vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(parserConfigPath));
    }
    function showUploadedImages() {
        const imageHistoryFilePath = mume_1.utility.addFileProtocol(path.resolve(mume_1.utility.extensionConfigDirectoryPath, './image_history.md'));
...
`);
    ...
    function insertImageUrl(uri, imageUrl) {
        ...
        vscode.window.visibleTextEditors
            .filter(editor => preview_content_provider_1.isMarkdownFile(editor.document) && editor.document.uri.fsPath === sourceUri.fsPath)
            .forEach(editor => {
    ...
    */
    ...
    context.subscriptions.push(vscode.window.onDidChangeTextEditorSelection(event => {
        if (preview_content_provider_1.isMarkdownFile(event.textEditor.document)) {
            const previewUri = preview_content_provider_1.getPreviewUri(event.textEditor.document.uri);
            vscode.commands.executeCommand('_workbench.htmlPreview.postMessage', previewUri, {
                command: 'changeTextEditorSelection',
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor((textEditor) => {
        if (textEditor && textEditor.document && textEditor.document.uri) {
            ...
            if (preview_content_provider_1.isMarkdownFile(textEditor.document)) {
                 ...
                 */
                if (preview_content_provider_1.useSinglePreview() && contentProvider.isPreviewOn(sourceUri)) {
