...
const fileAccess_1 = require("@abstractions/fileAccess");
...
function initLog() {
    ...
    if (!fileAccess_1.fileAccess.existsSync(logPath)) {
    ...
    try {
        logFd = fileAccess_1.fileAccess.openSync(logFilename, 'a');
    }
    catch (e) {
...
function log(message, level) {
    ...
    if (logFd) {
        try {
            fileAccess_1.fileAccess.appendFileSync(logFd, logEntry + '\n', 'utf8');
        }
        catch (e) {
...
try {
    fileAccess_1.fileAccess.closeSync(logFd);
}
catch (e) {
