...
class ExponentHelper {
    ...
    loginToExponent(promptForInformation, showMessage) {
        ...
        return XDL.currentUser()
            ...
            if (!user) {
                let username = "";
                return showMessage(localize(2, null))
                    .then(() => promptForInformation(localize(3, null), false)).then((name) => {
                    username = name;
                    return promptForInformation(localize(4, null), true);
                })
    ...
    isExpoApp(showProgress = false, logIfExpoIsNotInstalled = false) {
        if (showProgress) {
            this.logger.logStream("...");
        }
        ...
        return this.fs.readFile(packageJsonPath)
            ...
            if (!isExp && logIfExpoIsNotInstalled) {
                ...
                this.logger.logStream("\n");
                this.logger.logStream(localize(5, null));
                this.logger.logStream("\n");
            }
            return isExp;
        }).catch(() => {
